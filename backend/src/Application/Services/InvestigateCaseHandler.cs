...
public sealed record InvestigateCaseRequest(Guid RunId, Guid CaseId, string? ModelDeploymentName = null, float? Temperature = null);
...
public sealed class InvestigateCaseHandler
{
    ...
    public async Task<Result> HandleAsync(InvestigateCaseRequest request, CancellationToken cancellationToken)
    {
        ...
        var investigation = await _investigator.InvestigateAsync(run, caseUnderReview, request.ModelDeploymentName, request.Temperature, cancellationToken);
